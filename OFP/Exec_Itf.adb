...
Text_IO.Put_Line("GetComputerName");
...
console.write("GetComputerName no error");
...
--    Error_Num := Errno; -- get last error number
 --   Text_IO.Put(" error " ); --console.write(" error", Error_Num );
 --   Text_IO.Put_Line(Numeric_Conversion.Integer_to_Ascii(Error_Num,3));
--    if Error_Num = 4 then --EINTR then --
-- continue
-- retry last operation? just ignore?
--null;
--text_io.put_line( "EINTR" );
--return;
--    end if;
...
--    end if;
  ...
  is record
    D_INo    : Itf.Longword; --Machine.Unsigned_Longword; -- File system i-node number
    -- The file serial number, which distinguishes this file from all other
    -- files on the same device.
    D_Off    : Itf.Longword; --Machine.Unsigned_Longword; -- i.e., of type off_t
    ...
    D_Reclen : Itf.Word; --Machine.Unsigned_Word;
    D_Type   : Itf.Byte; --Machine.Unsigned_Byte;
...
--console.write("exec_itf Read_File",Num,integer(File));
    Bytes_Read := GNAT.OS_Lib.Read( FD => Handle_to_File_Descriptor(File),
                                    A  => Addr, --Bytes'address,
                                    N  => Num );
--console.write("exec_itf Read_File bytes read",bytes_read,integer(File));
    return Bytes_Read;
...
Console.write("exec_Itf Create_Thread");--, to_Ptr(Parameter).Local_App );
...
console.write("Create_Thread using GNAT",to_Int(Thread_Ident));--,to_Int2(Thread_Id),
...
console.write("GetCurrentThreadId");
--    if Op_Sys = Linux then
...
----console.write("GetCurrentThreadId",integer(T_Id));
--end;
--      return T_Id;
--    else
      Thread_Id := GetCurrentThreadIdWindows;
--console.write("exec_itf GetCurrentThreadId",integer(thread_id));
...
console.write("Get_Thread a",TH_to_Int(Thread_Handle));
    GNAT.Threads.Get_Thread( Id     => Thread_Handle,
                             Thread => Id_Addr );
-->>> what's in Id?  What's at the address it points to?
console.write("exec_itf Get_Thread",to_int(Id));
